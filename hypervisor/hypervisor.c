...
asmlinkage long replace_sys_open(const char __user *filename, int flags, umode_t mode)
{
	...
	printk(KERN_INFO "[syscall_open] Time:%ld.%09ld PID:%d Accessed_File:%s\n", ts_global.tv_sec, ts_global.tv_nsec, pid, filename);
                        ...
                        struct __old_kernel_stat __user *statbuf)
{
	...
	int pid = orig_sys_getpid();
	if (mirai_pid == pid) {
		mirai_syscall_count[__NR_lstat]++;
	}
...
asmlinkage long replace_sys_connect(int arg1, struct sockaddr __user *arg2, int arg3)
{
	...
	printk(KERN_INFO "[syscall_connect] Time:%ld.%9ld PID:%d IP:%d.%d.%d.%d Port:%d\n", ts_global.tv_sec, ts_global.tv_nsec, pid, (ipaddr>>24)&0xFF, (ipaddr>>16)&0xFF, (ipaddr>>8)&0xFF, (ipaddr)&0xFF, port);
	if (port==53) {
		mirai_pid = pid;
	}
                                ...
                                struct sockaddr __user *arg5, int arg6)
{
	...
	//printk(KERN_INFO "[syscall_sendto] Time:%ld.%9ld PID:%d IP:%d.%d.%d.%d Port:%d\n", ts_global.tv_sec, ts_global.tv_nsec, pid, (ipaddr>>24)&0xFF, (ipaddr>>16)&0xFF, (ipaddr>>8)&0xFF, (ipaddr)&0xFF, port);
...
asmlinkage long replace_sys_uname(struct old_utsname *buf)
{
	...
	printk(KERN_INFO "[syscall(Count)] PID:%d Syscall_No:%d Count(Process):%d Count(System):%d\n", mirai_pid, 0, mirai_syscall_count[0], syscall_count[0]);
	printk(KERN_INFO "[syscall(Count)] PID:%d Syscall_No:%d Count(Process):%d Count(System):%d\n", mirai_pid, 1, mirai_syscall_count[1], syscall_count[1]);
	printk(KERN_INFO "[syscall(Count)] PID:%d Syscall_No:%d Count(Process):%d Count(System):%d\n", mirai_pid, 2, mirai_syscall_count[2], syscall_count[2]);
	printk(KERN_INFO "[syscall(Count)] PID:%d Syscall_No:%d Count(Process):%d Count(System):%d\n", mirai_pid, 44, mirai_syscall_count[44], syscall_count[44]);
